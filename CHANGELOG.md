...
- **`/mella:commit` — removed `context: fork`** (mella plugin → v1.15.2). The forked subagent context blocked the `push` and `pr` flags outright: pushes and `gh pr create` calls made inside the fork couldn't complete, so those modes never ran. The skill now executes in the main conversation context; the Sonnet/low-effort pins stay so it remains cheap.
